...
#### FR-015: 打包方案
- **描述**: 支持将 Python 脚本打包为独立可执行文件。
- **工具**: PyInstaller。
- **要求**:
    - Windows: 生成 .exe，包含图标。
    - Linux: 生成单文件二进制。
    - 依赖: 需说明如何处理 `jpegoptim` 等外部工具（不强制打包，但需提示）。
...
#### FR-018: 自动化发布 (CI/CD)
- **描述**: 利用 GitHub Actions 实现版本自动化发布。
- **触发条件**: 当 Git 仓库推送以 `v` 开头的标签 (如 `v1.0.0`) 时。
- **执行动作**:
    1. 检出代码。
    2. 配置 Python 环境。
    3. 安装构建依赖 (PyInstaller)。
    4. 执行构建脚本 (`build_deb.py`)。
    5. 创建 GitHub Release。
    6. 上传生成的二进制文件和安装包。
- **验收标准**: 推送 Tag 后，Release 页面出现对应的安装包。

#### FR-019: 高 DPI 适配
- **描述**: 解决高分辨率屏幕下界面模糊或字体过小的问题。
- **详细逻辑**:
    1. 启动时检测操作系统。
    2. Windows: 调用 API 告知系统程序是 DPI Aware 的。
    3. Linux: 尝试读取 `Xft.dpi` 或调整 `tk scaling`。
- **验收标准**: 在 200% 缩放的屏幕上，界面元素清晰且大小适中。

#### FR-020: 品牌一致性
- **描述**: 统一应用程序的命名显示。
- **详细要求**:
    - 窗口标题应显示为 "TerryOptImg - Image Optimizer"。
...
- **详细要求**:
    - 包含 "IMG Opt" 或类似标识。
    - Windows: 嵌入 .exe 文件 (.ico)。
    - Linux: 安装到系统图标目录，并在 .desktop 文件中引用。
- **验收标准**: 打包后的程序在文件管理器和任务栏显示正确图标。

#### FR-023: 安装流程完善
- **描述**: 优化安装后的用户体验。
- **详细要求**:
    - 确保 Linux 安装后图标缓存刷新 (`update-icon-caches`)。
    - 确保桌面快捷方式可用。

#### FR-024: 动态 UI 缩放
- **描述**: 解决界面元素过小的问题，提供更友好的视觉体验。
- **详细要求**:
    - 检测屏幕 DPI。
    - 动态计算基础字体大小 (Base Font Size)。
    - 全局应用字体设置。
    - 增加控件间距。
- **验收标准**: 在 4K 屏幕上，字体清晰，按钮易点击。

#### FR-025: 手动 UI 缩放
- **描述**: 允许用户自定义界面缩放比例。
- **详细逻辑**:
    1. 增加配置项 `ui_scale`。
    2. 若 `ui_scale > 0`，则使用该值作为缩放因子。
    3. 若 `ui_scale == 0` (默认)，则使用自动检测。
    4. 界面提供 1.0x, 1.25x, 1.5x, 2.0x 等选项。
- **验收标准**: 修改设置重启后，界面大小发生变化。
